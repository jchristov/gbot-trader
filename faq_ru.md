...
Например для биржи WEX, торговая пара btc/usd:
...
```
echo off
cd /D E:\директория_с_ботом              - если start.bat файл находится в папке с ботом, 
                                           то эта строка не обязательна
...
### 53. Параметры STOP_LOSS_PERCENT, STOP_LOSS_ABS и BUY_STOP_ABS.
...
3. Параметр **BUY_STOP_ABS**
    ...
    Buy ордер выставиться на 1% выше текущего bid price, чтобы осуществить покупку по рынку.
